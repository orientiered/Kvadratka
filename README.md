...
+ [Технические особенности](#технические-особенности)
    + [Функция решения уравнения](#функция-решения-уравнения)
    + [Формат описания юнит-тестов](#формат-описания-юнит-тестов)
+ [Документация](#документация-doxygen)
...
5. Поддерживается экспоненциальная форма, а также nan и inf
...
(если осталось ровно 3 аргумента и программа не может идентифицировать первый как знакомый флаг, то она будет интерпретировать их как коэффициенты)
...
ZERO_ROOTS NAN NAN
```
В данном случае вместо `ZERO_ROOTS` можно было написать просто `0`

Здесь 0 корней, поэтому в полях `x1` и `x2` можно было написать любые числа, так как они не будут проверяться

## Документация Doxygen

Код документирован при помощи Doxygen

Чтобы сгенерировать документацию, воспользуйтесь командой `make doxygen` (см. [установка](#компиляция-и-запуск))

Создастся папка (по умолчанию) `doxDocs`.

Чтобы открыть докментацию, откройте браузером файл `index.html` в `doxDocs/html`
